...
public class AutocompletionlTextFieldCustom < T > extends TextField {
	// Local variables
	...
	public AutocompletionlTextFieldCustom() {
		super();
	...
	private void setListner() {
		...
		textProperty().addListener( ( observable, oldValue, newValue ) -> {
			...
			if ( enteredText == null || enteredText.isEmpty() ) {
				...
				// insensitive
				List < T > filteredEntries = entries.stream()
				...
				if ( !filteredEntries.isEmpty() ) {
					...
					if ( !entriesPopup.isShowing() ) { // optional
						entriesPopup.show( AutocompletionlTextFieldCustom.this, Side.BOTTOM, 0, 0 ); // position
						// of
						// popup
					}
					// no suggestions -> hide
	...
	private void populatePopup( List < T > searchResult, String searchReauest ) {
		...
		for ( int i = 0; i < count; i++ ) {
			final String result = ( String ) searchResult.get( i );
