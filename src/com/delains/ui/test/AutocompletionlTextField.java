...
public class AutocompletionlTextField extends TextField {
	...
	/**
	 * wrapper for default constructor with setting of "TextFieldWithLengthLimit"
	 * LengthLimit
	 *
	 * @param lengthLimit
	 */
	...
	public static TextFlow buildTextFlow( String text, String filter ) {
		...
		Text textFilter = new Text( text.substring( filterIndex, filterIndex + filter.length() ) ); // instead of
																									// "filter" to keep
																									// all "case
																									// sensitive"
		textFilter.setFill( Color.ORANGE );
		textFilter.setFont( Font.font( "Helvetica", FontWeight.BOLD, 12 ) );
		textFilter.setStyle( "-fx-background-color: white;" );
	...
	private void populatePopup( List < String > searchResult, String searchReauest ) {
		...
		for ( int i = 0; i < count; i++ ) {
			...
			item.setOnAction( actionEvent -> {
				setText( result );
				this.setText( result );
				setFinalFieldValue( result );
				AutoCompleteFieldObtainResult.setFieldResultObtained( result );
	...
	public String getFieldText() {
		return this.getFinalFieldValue();
	}

	private String finalFieldValue;

	public String getFinalFieldValue() {
		return finalFieldValue;
	}

	public void setFinalFieldValue( String finalFieldValue ) {
		this.finalFieldValue = finalFieldValue;
	}
