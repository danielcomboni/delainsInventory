...
public class UserFrame extends BorderPane {
	...
	private void saveChanges() {
		...
		System.out.println( "prev=" + userPrevious );
		...
		if ( StageForAlerts.isDecide() == true ) {
			...
			UserHibernation.updateUser( user, userPrevious.getId() );

			Refresh.setRefreshingDeterminant( 1 );

			UserManipulation.populateUserTable( tableViewUsers );

			AuditHistoryHibernation.auditValues( "edited user: " + user.getUserName(), UserLoggedIn.getUserLoggedIn() );
	...
	private void createNewUser() {
		...
		if ( StageForAlerts.isDecide() == true ) {
			...
			AuditHistoryHibernation.auditValues( "create new user: " + user.getUserName(),
	...
	public void deleteUser( BigDecimal userId ) {
		...
		if ( StageForAlerts.isDecide() == true ) {
			...
			AuditHistoryHibernation.auditValues( "deleted a user", UserLoggedIn.getUserLoggedIn() );
			AuditHistoryManipulation.populateTableWithRefreshing();
