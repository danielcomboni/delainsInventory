...
import com.delains.report.query.ReportQuerySalesCreditBetweenDates;
...
public class ReportSalesCreditBetweenDates extends BorderPane {
	...
	public static void setTableView( TableView < POS > tableView ) {
		ReportSalesCreditBetweenDates.tableView = tableView;
	}
	...
	private void buildComponents() {
		...
		datePickerFrom.setOnAction( e -> {
			...
			if ( datePickerTo.getValue() != null ) {
				...
				tableView.getItems().clear();
				tableView.setItems( ReportQuerySalesCreditBetweenDates.reportForAllSalesCreditBetweenDates( tableView,
						"credit sales between dates", dateFrom.toString(), dateTo.toString() ) );

				WorkBookUtils.setWorkbook( ReportQuerySalesCreditBetweenDates.getWorkBook() );
		...
		buttonExport.setOnAction( e -> {
			...
			String sheetTitle = "credit sales from " + dateBegin.toString() + " to " + dateEnd.toString() + " ("
					+ date.toString() + ")";
			...
			WorkBookUtils.writeWorkBookOut( WorkBookUtils.getSheetName(),
					ReceiptHeaderDAO.getReceiptHeader().getBusinessName() + "/credit sales between dates" );

		} );

	}

	public ReportSalesCreditBetweenDates() {

		this.setId( "main_borderpane" );

		getStylesheets().add( ReportSalesCreditBetweenDates.class.getResource( "report.css" ).toExternalForm() );
