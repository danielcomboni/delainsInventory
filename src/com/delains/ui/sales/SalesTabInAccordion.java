...
import com.delains.model.sales.DailyTotalSalesMainUI;
...
public class SalesTabInAccordion extends VBox {
	...
	private JFXButton buttonDaileSalesTotal;
	...
	public SalesTabInAccordion() {
		...
		buttonDaileSalesTotal = new JFXButton( "Daily sales total" );
		gridPane.add( buttonDaileSalesTotal, 0, 5 );
		...
		buttonDaileSalesTotal.setOnAction(e -> {
//			new StageForAllPopUps(new DailySalesTotalUI(), "Daily sales total").showAndWait();
			GeneralDialog.showDialog("Total Sales", "Total Sales", new DailyTotalSalesMainUI());
		});
...
POS p = AllSalesFrame.getTableView().getSelectionModel().getSelectedItem();
	...
	private void saveReturned() {
		...
		AuditHistory auditHistory = AuditHistoryHibernation.auditValues( " a sale returned ", UserLoggedIn.getUserLoggedIn() );
